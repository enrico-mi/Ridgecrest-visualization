let mapSketch = function(p){
    ...
    //const localAddress = 'https://ridgecrest-viz.s3.us-east-2.amazonaws.com/';
    const localAddress = './';

    p.preload = function() {
	// Load the GeoJSON data; you can convert shp to geojson at
	// https://mapshaper.org/
	Ponti_mechs = p.loadJSON(localAddress + 'geojson/ponti_mechs.geojson');
	Ponti = p.loadJSON(localAddress + 'geojson/ponti_all_lengths.geojson');
	//Ponti = p.loadJSON('./geojson/prova.geojson');
	TJ = p.loadJSON(localAddress + 'geojson/tj_geojson_ids.json');
	...
	return new Promise(resolve => {
	    ...
	    if (failMode != 2){
    		var geoJSONFileUrl = localAddress + 'geojson/streamlines/' +
